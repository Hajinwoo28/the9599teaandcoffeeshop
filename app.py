...
<body>
    ...
    <div id="size-modal" class="modal">
        <div class="modal-content" style="text-align:left;">
            ...
            <label style="font-weight:800; color:#8D6E63; font-size:0.8rem; text-transform:uppercase;">2. Add-ons (+₱10 each)</label>
            <div style="display:flex; flex-direction:column; gap:10px; margin-top:5px;">
                <label class="addon-label"><input type="checkbox" class="addon-checkbox" value="Nata"> Nata</label>
                <label class="addon-label"><input type="checkbox" class="addon-checkbox" value="Pearl"> Pearl</label>
                <label class="addon-label"><input type="checkbox" class="addon-checkbox" value="Coffee Jelly"> Coffee Jelly</label>
            </div>
                    ...
                    cartItemsList.innerHTML += `
                        <div class="cart-item">
                            <div class="item-details">
                                <h4>${escapeHTML(item.name)} <span style="color:#6F4E37; font-size:0.8rem;">${sizeLabel}</span>${addonLabel}</h4>
            ...
            try {
                ...
                data.orders.forEach(order => {
                    let itemsHtml = order.items.map(i => {
                        ...
                        return `<div style="margin-bottom:4px;"><strong>${escapeHTML(i.foundation)}${sizeDisplay}</strong>${addonDisplay} <span class="kds-badge" style="background:#EFEBE4; color:#5D4037;">${escapeHTML(i.pearls)}</span></div>`;
                    }).join('');
                ...
                quickCart.forEach((item, index) => {
                    ...
                    list.innerHTML += `
                        <div class="qo-cart-item">
                            <div>
                                <div style="font-weight:700; color:#3E2723; font-size:0.9rem;">${escapeHTML(item.name)}${sizeLabel}${addonLabel}</div>
            ...
            try {
                ...
                if(res.ok) {
                    ...
                    quickCart.forEach(item => {
                        let sizeLabel = item.size === 'Regular' ? '' : ` (${escapeHTML(item.size)})`;
                        let addonLabel = item.addons && item.addons.length > 0 ? ` (+ ${escapeHTML(item.addons.join(', '))})` : '';
                        receiptBody += `<div style="display:flex; justify-content:space-between;"><span>1x ${escapeHTML(item.name)}${sizeLabel}${addonLabel}</span><span>${item.price.toFixed(2)}</span></div>`;
                    });
...
def google_auth():
    ...
    Requires no SMS fees and guarantees real emails/names.
    """
    data = request.json
...
def api_orders():
    ...
    for res in all_reservations:
        items =[{'foundation': i.foundation, 'pearls': i.pearls, 'size': i.cup_size, 'addons': i.addons} for i in res.infusions]
