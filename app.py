...
.sidebar-toggle:hover{background:rgba(200,168,75,0.18);border-color:rgba(200,168,75,0.7);box-shadow:0 0 18px rgba(200,168,75,0.22);}
.sidebar-toggle.is-open{background:rgba(200,168,75,0.2);border-color:var(--gold);}
.sidebar-toggle-badge{position:absolute;top:-5px;right:-5px;background:var(--red);color:#fff;border-radius:50%;min-width:18px;height:18px;padding:0 3px;font-size:0.54rem;font-weight:900;display:none;align-items:center;justify-content:center;border:2.5px solid var(--teal-dark);box-shadow:0 2px 8px rgba(211,47,47,0.4);}
.sidebar-toggle-badge.show{display:flex;animation:badgePop 0.3s cubic-bezier(0.34,1.56,0.64,1);}
@keyframes badgePop{from{transform:scale(0);}to{transform:scale(1);}}
/* ── SIDE NAV BACKDROP ── */
.nav-backdrop{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);z-index:8998;}
.nav-backdrop.open{display:block;animation:backdropIn 0.28s ease;}
@keyframes backdropIn{from{opacity:0;}to{opacity:1;}}

/* ══════════════════════════════════════════
   PREMIUM EMPLOYEE SIDEBAR
══════════════════════════════════════════ */
.emp-nav-drawer{
  position:fixed;top:0;left:0;height:100%;width:248px;
  background:linear-gradient(175deg,#061F1B 0%,#082E28 30%,#0A3830 65%,#072620 100%);
  z-index:8999;
...
.emp-nav-drawer::before{
  content:'';position:absolute;inset:0;pointer-events:none;
  background:
    radial-gradient(ellipse 160px 120px at 20% 15%,rgba(200,168,75,0.06) 0%,transparent 70%),
    radial-gradient(ellipse 200px 150px at 85% 80%,rgba(13,122,106,0.12) 0%,transparent 70%),
    radial-gradient(ellipse 100px 80px at 10% 85%,rgba(200,168,75,0.04) 0%,transparent 70%);
}
/* Thin gold left-edge stripe */
.emp-nav-drawer::after{
  content:'';position:absolute;top:0;left:0;width:3px;height:100%;
  background:linear-gradient(180deg,transparent 0%,rgba(200,168,75,0.5) 20%,rgba(200,168,75,0.7) 50%,rgba(200,168,75,0.5) 80%,transparent 100%);
}
...
.drawer-logo-ring{
  ...
  background:linear-gradient(135deg,rgba(200,168,75,0.4) 0%,rgba(200,168,75,0.1) 100%);
  padding:2.5px;flex-shrink:0;
  box-shadow:0 0 18px rgba(200,168,75,0.2),inset 0 0 8px rgba(200,168,75,0.1);
}
.drawer-logo-circle{width:100%;height:100%;border-radius:50%;overflow:hidden;background:#fff;display:flex;align-items:center;justify-content:center;}
.drawer-logo-circle img{width:100%;height:100%;object-fit:cover;}
.drawer-brand-name{font-family:'Playfair Display',serif;font-size:0.97rem;font-weight:900;color:#fff;line-height:1.2;letter-spacing:0.2px;}
.drawer-brand-sub{font-size:0.58rem;color:var(--gold);font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-top:2px;opacity:0.85;}
.drawer-close{
...
.drawer-close:hover{background:rgba(255,255,255,0.13);color:#fff;border-color:rgba(255,255,255,0.25);}

/* Staff pill in header */
.drawer-staff-pill{
...
.drawer-staff-dot{width:8px;height:8px;border-radius:50%;background:#4CAF50;box-shadow:0 0 8px rgba(76,175,80,0.7);flex-shrink:0;animation:dotPulse 2.5s infinite;}
@keyframes dotPulse{0%,100%{opacity:1;box-shadow:0 0 6px rgba(76,175,80,0.7);}50%{opacity:0.7;box-shadow:0 0 12px rgba(76,175,80,0.9);}}
.drawer-staff-info{flex:1;min-width:0;}
.drawer-staff-role{font-size:0.66rem;font-weight:900;color:var(--gold);letter-spacing:1.5px;text-transform:uppercase;}
.drawer-staff-status{font-size:0.72rem;font-weight:700;color:rgba(255,255,255,0.55);margin-top:1px;}
...
.drawer-section-label::after{content:'';flex:1;height:1px;background:rgba(255,255,255,0.07);}

/* ── Nav List ── */
.drawer-nav{padding:6px 10px 0;flex:1;display:flex;flex-direction:column;gap:2px;overflow-y:auto;scrollbar-width:none;}
.drawer-nav::-webkit-scrollbar{display:none;}

/* ── Nav Items ── */
.drawer-nav-item,.emp-nav-item{
...
.drawer-nav-item:hover:not(.active),.emp-nav-item:hover:not(.active){
  background:rgba(255,255,255,0.05);
  color:rgba(255,255,255,0.88);
  border-color:rgba(255,255,255,0.07);
}
.drawer-nav-item.active,.emp-nav-item.active{
  background:linear-gradient(135deg,rgba(200,168,75,0.14) 0%,rgba(200,168,75,0.06) 100%);
  border-color:rgba(200,168,75,0.28);
  color:#fff;
  box-shadow:0 4px 20px rgba(200,168,75,0.08),inset 0 1px 0 rgba(200,168,75,0.1);
}
.drawer-nav-item.active::before,.emp-nav-item.active::before{height:60%;}
.drawer-nav-item:hover:not(.active)::before,.emp-nav-item:hover:not(.active)::before{height:30%;opacity:0.5;}
...
.drawer-nav-icon{
  ...
  transition:all 0.22s;color:rgba(255,255,255,0.5);
}
.drawer-nav-item.active .drawer-nav-icon,.emp-nav-item.active .drawer-nav-icon{
...
.drawer-nav-item:hover:not(.active) .drawer-nav-icon,.emp-nav-item:hover:not(.active) .drawer-nav-icon{
...
.drawer-nav-label{font-size:0.87rem;font-weight:800;letter-spacing:0.1px;line-height:1;color:inherit;}
.drawer-nav-desc{font-size:0.65rem;font-weight:600;color:rgba(255,255,255,0.3);margin-top:3px;line-height:1.3;}
.drawer-nav-item.active .drawer-nav-desc,.emp-nav-item.active .drawer-nav-desc{color:rgba(255,255,255,0.5);}
.drawer-nav-item:hover:not(.active) .drawer-nav-desc,.emp-nav-item:hover:not(.active) .drawer-nav-desc{color:rgba(255,255,255,0.4);}
/* Nav badges */
.drawer-nav-badge,.nav-item-badge{background:var(--red);color:#fff;border-radius:20px;min-width:20px;height:20px;padding:0 5px;font-size:0.6rem;font-weight:900;display:none;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(211,47,47,0.4);}
.drawer-nav-badge.show,.nav-item-badge.show{display:flex;}

/* Divider */
.drawer-divider{height:1px;background:rgba(255,255,255,0.06);margin:8px 16px;}

/* ── Drawer Footer ── */
.drawer-footer{padding:14px 12px 16px;border-top:1px solid rgba(255,255,255,0.07);flex-shrink:0;}
.drawer-time-chip{
  display:flex;align-items:center;gap:8px;
  background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);
  border-radius:10px;padding:8px 12px;margin-bottom:10px;
}
.drawer-time-chip i{color:var(--gold);font-size:0.75rem;opacity:0.7;}
.drawer-time-val{font-size:0.75rem;font-weight:800;color:rgba(255,255,255,0.45);letter-spacing:0.3px;}
.drawer-logout{
  ...
  cursor:pointer;font-family:'Nunito',sans-serif;font-size:0.83rem;font-weight:800;
  text-decoration:none;transition:all 0.2s;
}
.drawer-logout:hover{background:rgba(211,47,47,0.2);color:#fff;border-color:rgba(211,47,47,0.5);box-shadow:0 4px 16px rgba(211,47,47,0.15);}
...
.page-header{padding:12px 14px 10px;background:var(--card);border-bottom:1.5px solid var(--border);position:sticky;top:0;z-index:50;}
.page-header h2{font-family:'Playfair Display',serif;font-size:1.05rem;font-weight:900;color:var(--teal-dark);display:flex;align-items:center;gap:9px;}
.page-header p{font-size:0.7rem;color:var(--muted);margin-top:2px;font-weight:600;}

.section{padding:10px 12px 0;}
.card{background:var(--card);border-radius:var(--radius);border:1.5px solid var(--border);box-shadow:var(--shadow);padding:12px;margin-bottom:10px;}
.card-title{font-size:0.74rem;font-weight:900;color:var(--teal-dark);text-transform:uppercase;letter-spacing:0.8px;margin-bottom:8px;display:flex;align-items:center;gap:7px;}

.stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px;}
.stat-chip{background:var(--card);border:1.5px solid var(--border);border-radius:10px;padding:9px 8px;text-align:center;box-shadow:var(--shadow);}
.stat-num{font-family:'Playfair Display',serif;font-size:1.25rem;font-weight:900;color:var(--teal-dark);line-height:1;}
.stat-lbl{font-size:0.58rem;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:0.6px;margin-top:2px;}
...
.pos-menu-area{flex:1;min-height:0;overflow:hidden;display:flex;flex-direction:column;padding:8px;background:var(--bg);scrollbar-width:none;}
.pos-menu-area::-webkit-scrollbar{display:none;}
.pos-sidebar{width:230px;flex-shrink:0;background:var(--card);border-left:1.5px solid var(--border);display:flex;flex-direction:column;overflow:hidden;}
@media(max-width:680px){
  .pos-layout{flex-direction:column;height:calc(100vh - var(--topbar-h) - var(--nav-h));overflow:hidden;}
  .pos-menu-area{flex:0 0 48%;overflow-y:auto;padding:8px;min-height:0;}
  .pos-sidebar{flex:0 0 52%;width:100%;border-left:none;border-top:2px solid var(--border);min-height:0;}
}

.pos-cat-tabs{display:flex;gap:4px;overflow-x:auto;padding:0 0 8px;scrollbar-width:none;margin-bottom:3px;flex-shrink:0;background:var(--bg);}
.pos-cat-tabs::-webkit-scrollbar{display:none;}
.cat-tab{flex-shrink:0;padding:5px 11px;border-radius:20px;border:1.5px solid var(--border);background:var(--card);font-size:0.67rem;font-weight:800;color:var(--muted);cursor:pointer;transition:all 0.15s;}
.cat-tab.active{background:var(--teal-dark);color:#fff;border-color:var(--teal-dark);}

.menu-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(96px,1fr));gap:7px;flex:1;overflow-y:auto;align-content:start;min-height:0;scrollbar-width:none;}
.menu-grid::-webkit-scrollbar{display:none;}
.menu-card{position:relative;background:var(--card);border:1.5px solid var(--border);border-radius:10px;padding:0 0 6px;cursor:pointer;transition:all 0.15s;text-align:center;box-shadow:0 2px 8px rgba(13,122,106,0.06);overflow:hidden;}
.menu-bs-tag{position:absolute;top:4px;left:4px;right:auto;z-index:2;background:rgba(200,168,75,0.95);color:#0A2925;font-size:0.5rem;font-weight:900;padding:2px 6px;border-radius:8px;letter-spacing:0.3px;}
.menu-card:active{transform:scale(0.97);opacity:0.85;}
.menu-card.oos{opacity:0.45;cursor:not-allowed;}
.menu-preview{width:100%;height:60px;object-fit:cover;display:block;border-radius:8px 8px 0 0;background:var(--teal-light);}
.menu-letter{width:100%;height:60px;border-radius:8px 8px 0 0;display:flex;align-items:center;justify-content:center;font-size:1.5rem;}
.menu-card-body{padding:4px 5px 0;}
.menu-name{font-size:0.63rem;font-weight:800;color:var(--text);line-height:1.2;margin-bottom:2px;}
.menu-price{font-size:0.68rem;font-weight:900;color:var(--teal-mid);}
.menu-oos-tag{background:rgba(211,47,47,0.1);color:var(--red);font-size:0.52rem;font-weight:800;padding:1px 5px;border-radius:8px;margin-top:2px;display:inline-block;}

.cart-header{padding:9px 12px;border-bottom:1.5px solid var(--border);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}
.cart-title{font-size:0.79rem;font-weight:900;color:var(--teal-dark);display:flex;align-items:center;gap:6px;}
.cart-clear{background:none;border:none;color:var(--red);font-size:0.68rem;font-weight:800;cursor:pointer;padding:3px 7px;border-radius:6px;}
.cart-clear:hover{background:rgba(211,47,47,0.08);}
.cart-items{flex:1;overflow-y:auto;padding:8px 10px;scrollbar-width:none;}
.cart-items::-webkit-scrollbar{display:none;}
.cart-item{display:flex;align-items:flex-start;gap:7px;padding:7px 0;border-bottom:1px solid var(--border);}
.cart-item:last-child{border-bottom:none;}
.cart-item-info{flex:1;min-width:0;}
.cart-item-name{font-size:0.74rem;font-weight:800;color:var(--text);line-height:1.3;}
.cart-item-mods{font-size:0.62rem;color:var(--muted);margin-top:1px;line-height:1.4;}
...
.cart-footer{padding:8px 10px;border-top:1.5px solid var(--border);flex-shrink:0;}
.cart-total-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;}
.cart-total-lbl{font-size:0.68rem;font-weight:800;color:var(--muted);text-transform:uppercase;}
.cart-total-amt{font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:900;color:var(--teal-dark);}
.name-inp{width:100%;padding:7px 10px;border:1.5px solid var(--border);border-radius:9px;font-family:'Nunito',sans-serif;font-size:0.78rem;font-weight:700;outline:none;color:var(--text);margin-bottom:6px;background:#fff;}
.name-inp:focus{border-color:var(--teal);}
.btn-checkout{width:100%;padding:9px;border-radius:10px;border:none;background:var(--teal-dark);color:#fff;font-family:'Nunito',sans-serif;font-size:0.82rem;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;letter-spacing:0.3px;transition:opacity 0.15s;}
.btn-checkout:active{opacity:0.85;}
.btn-checkout:disabled{opacity:0.5;cursor:not-allowed;}

/* ── TABLE LAYOUT (Live Orders screen) ── */
.live-section{padding:10px 12px 0;}
.live-card{background:var(--card);border:1.5px solid var(--border);border-radius:var(--radius);box-shadow:var(--shadow);margin-bottom:10px;overflow:hidden;}
.live-card-header{display:flex;align-items:center;justify-content:space-between;padding:9px 13px;border-bottom:1.5px solid var(--border);}
.live-card-title{font-size:0.76rem;font-weight:900;color:var(--teal-dark);display:flex;align-items:center;gap:7px;text-transform:uppercase;letter-spacing:0.6px;}
.live-card-title.perm-title{color:var(--red);}
.btn-refresh{background:var(--red);color:#fff;border:none;padding:5px 13px;border-radius:7px;font-family:'Nunito',sans-serif;font-size:0.72rem;font-weight:900;cursor:pointer;letter-spacing:0.3px;}
...
.live-table{width:100%;border-collapse:collapse;font-size:0.65rem;table-layout:fixed;}
...
.sa-body{padding:10px 12px 16px;}
.sa-card{background:var(--card);border-radius:10px;border:1.5px solid var(--border);padding:10px 13px;margin-bottom:8px;display:flex;align-items:center;gap:11px;box-shadow:var(--shadow);}
...
.sa-name{font-weight:800;font-size:0.82rem;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.sa-lvl{font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-top:1px;}
.sa-lvl.critical{color:var(--red);}
.sa-lvl.low{color:var(--orange);}
.sa-lvl.medium{color:var(--blue);}
.sa-qty{font-size:0.84rem;font-weight:900;color:var(--text);text-align:right;flex-shrink:0;}
.sa-unit{font-size:0.63rem;font-weight:700;color:var(--muted);display:block;text-align:right;}
...
/* ── Bell button ── */
.bell-btn{background:rgba(255,255,255,0.1);border:1px solid rgba(200,168,75,0.3);color:var(--gold);width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer;position:relative;font-size:0.9rem;flex-shrink:0;transition:background 0.15s;}
...
.bell-count.show{display:flex;animation:bellPulse 1.5s infinite;}
@keyframes bellPulse{0%,100%{box-shadow:0 0 0 0 rgba(211,47,47,0.5);}50%{box-shadow:0 0 0 5px rgba(211,47,47,0);}}

/* ── Notification panel (fixed dropdown from bell) ── */
.notif-panel{position:fixed;top:70px;right:10px;width:350px;max-width:calc(100vw - 20px);background:#fff;border-radius:18px;box-shadow:0 20px 60px rgba(0,0,0,0.18),0 4px 16px rgba(13,122,106,0.1);z-index:9999;overflow:hidden;display:none;border:1px solid rgba(13,122,106,0.12);}
.notif-panel.open{display:block;animation:notifDrop 0.2s cubic-bezier(0.34,1.56,0.64,1);}
@keyframes notifDrop{from{opacity:0;transform:translateY(-10px) scale(0.97);}to{opacity:1;transform:translateY(0) scale(1);}}
.notif-header{background:linear-gradient(135deg,var(--teal-dark) 0%,var(--teal-mid) 100%);color:#fff;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;}
.notif-header-title{font-weight:900;font-size:0.92rem;display:flex;align-items:center;gap:9px;}
...
.notif-clear{background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);color:#fff;font-size:0.68rem;font-weight:800;padding:5px 12px;border-radius:20px;cursor:pointer;font-family:'Nunito',sans-serif;transition:background 0.15s;}
...
.notif-icon{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:0.9rem;flex-shrink:0;margin-top:1px;}
.notif-icon.order{background:rgba(21,101,192,0.1);color:#1565C0;border:1px solid rgba(21,101,192,0.15);}
.notif-icon.perm{background:rgba(230,81,0,0.1);color:var(--orange);border:1px solid rgba(230,81,0,0.15);}
.notif-icon.stock-critical{background:rgba(211,47,47,0.1);color:var(--red);border:1px solid rgba(211,47,47,0.15);}
.notif-icon.stock-low{background:rgba(230,81,0,0.08);color:var(--orange);border:1px solid rgba(230,81,0,0.12);}
...
.notif-empty{padding:32px 16px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px;}
.notif-empty-icon{width:46px;height:46px;border-radius:50%;background:rgba(13,122,106,0.07);border:1.5px solid rgba(13,122,106,0.15);display:flex;align-items:center;justify-content:center;font-size:1.25rem;}
...
.notif-section-label{padding:7px 16px 5px;font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:1.2px;color:#9ca3af;background:#f9fbfa;border-bottom:1px solid #edf2f1;display:flex;align-items:center;gap:6px;}
.notif-section-label::before{content:'';width:3px;height:11px;border-radius:2px;display:inline-block;}
...
<header class="topbar">
  ...
  <div class="topbar-right">
    ...
    <button class="sidebar-toggle" id="emp-hamburger" onclick="toggleEmpMenu()" title="Open navigation" aria-label="Open menu">
      <i class="fas fa-grip-lines"></i>
      <span class="sidebar-toggle-badge" id="hamburger-badge"></span>
    </button>
  </div>
</header>
...
<!-- PREMIUM EMPLOYEE SIDEBAR -->
<nav class="emp-nav-drawer" id="emp-nav-dropdown" aria-label="Employee Navigation">
  ...
  <div class="drawer-header">
    <div class="drawer-header-inner">
      ...
      <button class="drawer-close" onclick="closeEmpMenu()" aria-label="Close menu">
        <i class="fas fa-times"></i>
      </button>
    </div>

    <!-- Staff Status Pill -->
    <div class="drawer-staff-pill">
      <div class="drawer-staff-dot"></div>
      <div class="drawer-staff-info">
        <div class="drawer-staff-role">Staff</div>
        <div class="drawer-staff-status">Active &amp; on duty</div>
      </div>
    </div>
  </div>
    ...
    <div class="pos-layout">
      ...
      <div class="pos-sidebar">
        ...
        <div class="cart-footer">
          ...
          <input type="text" class="name-inp" id="customer-name" placeholder="Customer name *" oninput="renderCart()" required>
...
(function tickClock(){const el=document.getElementById('clock');const sl=document.getElementById('sidebar-clock');if(el){const now=new Date();const h=now.getHours(),m=now.getMinutes(),s=now.getSeconds();const h12=h%12||12;const ap=h<12?'AM':'PM';const t=`${h12}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')} ${ap}`;el.textContent=t;if(sl)sl.textContent=t;}setTimeout(tickClock,1000);})();
...
function toggleEmpMenu(){
  ...
  if(isOpen){dd.classList.remove('open');backdrop.classList.remove('open');if(btn)btn.classList.remove('is-open');}
  else{dd.classList.add('open');backdrop.classList.add('open');if(btn)btn.classList.add('is-open');}
}
...
const CATEGORY_EMOJI_MAP={
  'Milktea':       {em:'🧋', bg:'#EDE7F6'},
  'Coffee':        {em:'☕', bg:'#EFEBE9'},
  'Milk Series':   {em:'🥛', bg:'#E3F2FD'},
  'Matcha Series': {em:'🍵', bg:'#E8F5E9'},
  'Fruit Soda':    {em:'🍹', bg:'#FCE4EC'},
  'Frappe':        {em:'🥤', bg:'#E0F7FA'},
  'Snacks':        {em:'🍟', bg:'#FFF8E1'},
};
const ITEM_EMOJI_OVERRIDE={
  'Taro Milktea':'🧋','Okinawa Milktea':'🧋','Wintermelon Milktea':'🧋',
  'Cookies and Cream Milktea':'🍪','Matcha Milktea':'🍵','Dark Belgian Choco':'🍫',
  'Biscoff Milktea':'☕','Mocha':'☕','Caramel Macchiato':'🍮',
  'Iced Americano':'🧊','Cappuccino':'☕','Coffee Jelly Drink':'☕',
  'French Vanilla':'🍦','Hazelnut':'🌰','Ube Milk':'💜',
  'Mango Milk':'🥭','Strawberry Milk':'🍓','Blueberry Milk':'🫐',
  'Matcha Latte':'🍵','Matcha Caramel':'🍵','Matcha Strawberry':'🍵','Matcha Frappe':'🍵',
  'Lychee Mogu Soda':'🍹','Apple Soda':'🍏','Strawberry Soda':'🍓','Blueberry Soda':'🫐',
  'Cookies and Cream Frappe':'🍪','Mocha Frappe':'☕','Coffee Frappe':'☕',
  'Strawberry Frappe':'🍓','Mango Frappe':'🥭',
...
const POS_IMAGE_MAP={
  ...
  'Iced Americano':'/static/images/iced_americano.jpg',
  'Matcha Milktea':'/static/images/matcha_milktea.jpg',
  'Coffee Jelly Drink':'/static/images/coffee_jelly_drink.jpg',
};
...
function getPosImageTag(item){
  if(POS_IMAGE_MAP[item.name]){
    const cat=CATEGORY_EMOJI_MAP[item.category]||{em:'🧋',bg:'#E6F4F2'};
    const em=ITEM_EMOJI_OVERRIDE[item.name]||cat.em;
    return `<img class="menu-preview" src="${POS_IMAGE_MAP[item.name]}" alt="${escapeHTML(item.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">` +
           `<div class="menu-letter" style="display:none;background:${cat.bg};">${em}</div>`;
  }
  const cat=CATEGORY_EMOJI_MAP[item.category]||{em:'🧋',bg:'#E6F4F2'};
  const em=ITEM_EMOJI_OVERRIDE[item.name]||cat.em;
  return `<div class="menu-letter" style="background:${cat.bg};">${em}</div>`;
}
...
function renderMenuGrid(cat){
  ...
  grid.innerHTML=items.map(function(m){
    const imgContent=getPosImageTag(m);
    const bs=posCardStyle(m).badge==='bestseller'?'<div class="menu-bs-tag">\u2B50 Best</div>':'';
...
// Per-status sounds for customer order tracking
function _sfTones(notes) {
    var ctx = _getSfAudioCtx(); if (!ctx) return;
    if (ctx.state === 'suspended') ctx.resume();
    notes.forEach(function(t) {
        var osc = ctx.createOscillator(), gain = ctx.createGain();
        osc.connect(gain); gain.connect(ctx.destination);
        osc.type = t[3] || 'sine'; osc.frequency.value = t[1];
        var s = ctx.currentTime + t[0];
        gain.gain.setValueAtTime(0, s);
        gain.gain.linearRampToValueAtTime(t[4] || 0.32, s + 0.02);
        gain.gain.exponentialRampToValueAtTime(0.001, s + t[2]);
        osc.start(s); osc.stop(s + t[2]);
    });
}
function playStatusSound(status) {
    if (status === 'Preparing' || status === 'Preparing Order') {
        // Two quick ascending pulses — "we are on it!"
        _sfTones([[0, 440, 0.18], [0.22, 554, 0.22]]);
    } else if (status === 'Ready for Pickup' || status === 'Ready for Pick-up') {
        // Triumphant C-E-G fanfare — "come get it!"
        _sfTones([[0, 523, 0.22], [0.24, 659, 0.22], [0.48, 784, 0.38]]);
    } else if (status === 'Completed') {
        // Gentle descending wrap-up chime
        _sfTones([[0, 784, 0.2], [0.22, 659, 0.2], [0.44, 523, 0.32]]);
    } else if (status === 'Cancelled') {
        // Two low descending tones — somber
        _sfTones([[0, 330, 0.22, 'triangle', 0.3], [0.28, 247, 0.3, 'triangle', 0.25]]);
    } else {
        // Default single ding for any other status
        _sfTones([[0, 660, 0.25]]);
    }
}
...
<div class="loc-modal-overlay" id="loc-modal" onclick="if(event.target===this)closeLocModal()">
    <div class="loc-modal-sheet">
        ...
        <!-- Shop Photo -->
        <div class="loc-map-wrap" style="height:auto; border-radius:14px; overflow:hidden; margin-bottom:14px;">
            <img src="data:image/png;base64,"
                 alt="9599 Tea &amp; Coffee Storefront"
                 style="width:100%; height:auto; display:block; object-fit:cover; border-radius:14px;">
        </div>
...
:root{
  ...
  --radius:14px; --nav-h:64px; --topbar-h:52px;
...
/* ── ADMIN SIDE DRAWER ── */
.admin-nav-drawer{position:fixed;top:0;left:0;height:100%;width:248px;background:linear-gradient(170deg,#261408 0%,#3D2410 30%,#4A2C14 65%,#321A0A 100%);z-index:8999;transform:translateX(-100%);transition:transform 0.32s cubic-bezier(0.4,0,0.2,1);display:flex;flex-direction:column;box-shadow:8px 0 50px rgba(0,0,0,0.5);overflow:hidden;}
...
@media(min-width:768px){
  ...
  .page-header p{font-size:0.7rem !important;}
  /* Dashboard: 4-column KPI on wider screens */
  .dash-kpi-grid{grid-template-columns:repeat(4,1fr);gap:10px;padding:14px 14px 0;}
  .dash-kpi{padding:11px;}
  .dash-kpi-val{font-size:1.25rem;}
  .dash-hero{padding:14px 18px 26px;}
  .dash-hero-title{font-size:1.1rem;}
  /* Quick actions: 6 columns on desktop */
  .dash-qa-grid{grid-template-columns:repeat(6,1fr);gap:8px;}
  .dash-qa-btn{padding:10px 6px;gap:5px;}
  .dash-qa-icon{width:32px;height:32px;font-size:0.85rem;}
  .dash-qa-label{font-size:0.6rem;}
  ...
  /* Live orders: 2 columns at 768px */
  .lo-cards-grid{grid-template-columns:1fr 1fr;}
  .lo-stats-bar{grid-template-columns:repeat(4,1fr);}
  /* Section and screen-inner spacing */
  .section{margin:0 10px 6px;}
  .screen-inner{padding:12px 0 0;}
  /* Inventory stats: 4-col row */
  .inv-stats-row{grid-template-columns:repeat(4,1fr);}
  /* Extended screen content areas */
  #s-analytics [style*="max-height"],
  #s-promos [style*="max-height"],
  #s-announce [style*="max-height"],
  #s-waste [style*="max-height"],
  #s-customers [style*="max-height"],
  #s-checklist [style*="max-height"],
  #s-security [style*="max-height"]{max-height:calc(100vh - var(--topbar-h) - 80px) !important;}
}
@media(min-width:1100px){
  .lo-cards-grid{grid-template-columns:repeat(3,1fr);}
...
.admin-drawer-brand-sub{font-size:0.55rem;color:var(--tan);font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-top:1px;opacity:0.85;}
.admin-drawer-close{width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:0.75rem;transition:all 0.18s;flex-shrink:0;}
...
.admin-drawer-nav{padding:6px 10px 0;flex:1;display:flex;flex-direction:column;gap:3px;overflow-y:auto;scrollbar-width:none;}
.admin-drawer-nav::-webkit-scrollbar{display:none;}
.admin-nav-item{width:100%;display:flex;align-items:center;gap:13px;padding:13px 14px;border-radius:8px;border:1px solid transparent;background:transparent;color:rgba(196,168,130,0.55);cursor:pointer;font-family:'Nunito',sans-serif;text-align:left;transition:all 0.22s cubic-bezier(0.4,0,0.2,1);position:relative;overflow:hidden;}
...
.admin-nav-icon{width:44px;height:44px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:1.15rem;flex-shrink:0;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.07);transition:all 0.22s;color:rgba(196,168,130,0.55);}
...
.admin-nav-label{font-size:0.97rem;font-weight:800;letter-spacing:0.1px;line-height:1;color:inherit;}
.admin-nav-desc{font-size:0.72rem;font-weight:600;color:rgba(196,168,130,0.32);margin-top:3px;line-height:1.3;}
