        ...
        async function saveConfigurations() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    // Parse the closing time to show as the Expiration Date
                    const [hours, minutes] = closeTimeVal.split(':');
                    const expiryDate = new Date();
                    expiryDate.setHours(parseInt(hours), parseInt(minutes), 0, 0);
                    
                    // If the closing time picked is actually earlier than 'now', 
                    // it implies the store closes tomorrow at that time.
                    ...
                    alert('Settings validated! The link is now set to expire at your closing time: ' + closeTime);
                } else {
...
def storefront():
    ...
    try:
        # We decode without max_age first to read the closing time
        data = token_serializer.loads(token) 
        open_time = data.get('open', '06:00 AM')
        close_time = data.get('close', '07:00 PM')
        
        # Convert close_time (e.g. "07:00 PM") to a datetime object for today
        current_ph_time = get_ph_time()
        try:
            closing_dt = datetime.strptime(close_time, '%I:%M %p')
            # Combine today's date with the closing time
            expiry_dt = current_ph_time.replace(hour=closing_dt.hour, minute=closing_dt.minute, second=0, microsecond=0)
            
            # If current time is already past closing, the link is expired
            if current_ph_time > expiry_dt:
                return expired_html, 403
        except Exception as e:
            print(f"Time validation error: {e}")
            # Fallback to a safe 1-hour window if parsing fails
