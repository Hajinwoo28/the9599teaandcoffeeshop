...
<script>
    ...
    function addToCart(name, cat, price) {
        ...
        } else if (cat === 'Snacks' && name.toLowerCase().includes('fries')) {
            // French Fries — open flavor picker modal
            document.getElementById('fries-modal-title').innerText = name;
...
<div id="qo-size-modal" class="modal">
    <div class="modal-content">
        ...
        <div style="text-align:center; margin:14px 0 8px;">
            <div style="font-size:0.72rem; font-weight:800; color:#8D6E63; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">Quantity</div>
            <div style="display:inline-flex; align-items:center; background:#F5EFE6; border-radius:50px; border:1.5px solid #D7CCC8;">
                <button onclick="changeQOQty('qo-size-qty',-1)" style="width:38px;height:38px;border:none;background:transparent;font-size:1.3rem;font-weight:900;cursor:pointer;border-radius:50px;">−</button>
                <div id="qo-size-qty" style="min-width:36px;text-align:center;font-size:1.3rem;font-weight:900;color:#3E2723;">1</div>
                <button onclick="changeQOQty('qo-size-qty',1)" style="width:38px;height:38px;border:none;background:transparent;font-size:1.3rem;font-weight:900;cursor:pointer;border-radius:50px;">+</button>
            </div>
        </div>
        <div style="display:flex; gap:10px;">
...
<div id="qo-custom-modal" class="modal">
    <div class="modal-content">
        ...
        <div id="qo-custom-drink-options">
            <label style="font-size:0.75rem; font-weight:800; color:#8D6E63; text-transform:uppercase; letter-spacing:1px; margin-bottom:6px; display:block;">Sugar Level</label>
        ...
        <div style="text-align:center; margin:14px 0 8px;">
            <div style="font-size:0.72rem; font-weight:800; color:#8D6E63; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">Quantity</div>
            <div style="display:inline-flex; align-items:center; background:#F5EFE6; border-radius:50px; border:1.5px solid #D7CCC8;">
                <button onclick="changeQOQty('qo-custom-qty',-1)" style="width:38px;height:38px;border:none;background:transparent;font-size:1.3rem;font-weight:900;cursor:pointer;border-radius:50px;">−</button>
                <div id="qo-custom-qty" style="min-width:36px;text-align:center;font-size:1.3rem;font-weight:900;color:#3E2723;">1</div>
                <button onclick="changeQOQty('qo-custom-qty',1)" style="width:38px;height:38px;border:none;background:transparent;font-size:1.3rem;font-weight:900;cursor:pointer;border-radius:50px;">+</button>
            </div>
        </div>
        <div style="display:flex; gap:10px;">
...
<div id="qo-fries-modal" class="modal">
    <div class="modal-content">
        ...
        <div style="text-align:center; margin:14px 0 8px;">
            <div style="font-size:0.72rem; font-weight:800; color:#8D6E63; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">Quantity</div>
            <div style="display:inline-flex; align-items:center; background:#F5EFE6; border-radius:50px; border:1.5px solid #D7CCC8;">
                <button onclick="changeQOQty('qo-fries-qty',-1)" style="width:38px;height:38px;border:none;background:transparent;font-size:1.3rem;font-weight:900;cursor:pointer;border-radius:50px;">−</button>
                <div id="qo-fries-qty" style="min-width:36px;text-align:center;font-size:1.3rem;font-weight:900;color:#3E2723;">1</div>
                <button onclick="changeQOQty('qo-fries-qty',1)" style="width:38px;height:38px;border:none;background:transparent;font-size:1.3rem;font-weight:900;cursor:pointer;border-radius:50px;">+</button>
            </div>
        </div>
...
<script>
    ...
    function addQO(name, cat, price) {
        ...
        if (['Milktea', 'Coffee'].includes(cat)) {
            ...
            document.getElementById('qo-size-qty').innerText = '1';
            document.getElementById('qo-size-modal').style.display = 'flex';
        } else if (cat === 'Snacks' && name.toLowerCase().includes('fries')) {
            // French Fries — flavor picker
            document.getElementById('qo-fries-modal-title').innerText = name;
            document.querySelector('input[name="qo_fries_flavor"][value="Plain"]').checked = true;
            document.getElementById('qo-fries-qty').innerText = '1';
            document.getElementById('qo-fries-modal').style.display = 'flex';
        } else {
            // All other items — custom modal (sugar, ice, add-ons, no size)
            ...
            // Hide sugar/ice/addons for snacks — they don't apply
            document.getElementById('qo-custom-drink-options').style.display = isSnack ? 'none' : 'block';
            if (!isSnack) {
                document.getElementById('qo-custom-sugar').value = '100% Sugar';
                document.getElementById('qo-custom-ice').value = 'Normal Ice';
            }
            document.getElementById('qo-custom-qty').innerText = '1';
            document.getElementById('qo-custom-modal').style.display = 'flex';
        }
    }

    function changeQOQty(id, delta) {
        const el = document.getElementById(id);
        let v = parseInt(el.innerText) + delta;
    ...
    function confirmQuickCart() {
        ...
        const qty = parseInt(document.getElementById('qo-size-qty').innerText) || 1;
        const item = {
        ...
        for(let i = 0; i < qty; i++) qoCart.push({...item});
    ...
    function confirmCustomCart() {
        const isSnack = qoPending.cat === 'Snacks';
        let addons = []; let addonCost = 0;
        if (!isSnack) {
        ...
        const qty = parseInt(document.getElementById('qo-custom-qty').innerText) || 1;
        const item = {
            name: qoPending.name, size: 'Regular', price: qoPending.price + addonCost,
            sugar: isSnack ? 'N/A' : document.getElementById('qo-custom-sugar').value,
            ice:   isSnack ? 'N/A' : document.getElementById('qo-custom-ice').value,
            addons: addons.join(', ')
        };
        for(let i = 0; i < qty; i++) qoCart.push({...item});
    ...
    function confirmQOFries() {
        ...
        const qty = parseInt(document.getElementById('qo-fries-qty').innerText) || 1;
        for(let i = 0; i < qty; i++) {
            qoCart.push({name: `${qoPending.name} (${flavor})`, size: 'Regular', price: qoPending.price, sugar: 'N/A', ice: 'N/A', addons: ''});
        }
