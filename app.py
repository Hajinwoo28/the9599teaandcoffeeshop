...
<script>
    ...
    /* Per-item photo mapping → { src, pos }
       Product images stored in /static/images/ */
    const ITEM_PHOTOS = {
        ...
        'Matcha Caramel':         { src: '/static/images/matcha_caramel.png',            pos: '50% 50%' },
        ...
        'Matcha Latte':           { src: '/static/images/matcha_latte.png',              pos: '50% 50%' },
        ...
        'Cookies & Cream Frappe': { src: '/static/images/cookies_and_cream_frappe.jpg', pos: '50% 50%' },
    ...
    function updateNotifUI() {
        ...
        const STATUS_ICON = {
            'Waiting Confirmation': { icon: 'fa-clock',           cls: '',          color: '#7B1FA2' },
            'Preparing Order':      { icon: 'fa-fire',            cls: '',          color: '#E65100' },
            'Ready for Pick-up':    { icon: 'fa-check-circle',    cls: 'ready',     color: 'var(--green-ready)' },
            'Completed':            { icon: 'fa-flag-checkered',  cls: 'completed', color: 'var(--coffee)' },
            'Cancelled':            { icon: 'fa-times-circle',    cls: 'cancelled', color: 'var(--red-alert)' },
        };

        if (notifications.length > 0) {
            list.innerHTML = notifications.map(n => {
                const si = STATUS_ICON[n.status] || { icon: 'fa-info-circle', cls: '', color: 'var(--gold)' };
                return `<div class="notif-item ${si.cls}">
    ...
    function playCustomerNotificationSound(status) {
        try {
            if (!customerAudioCtx) initCustomerAudio();
            if (!customerAudioCtx || customerAudioCtx.state === 'suspended') return;
            function playTone(freq, time, duration) {
                const osc = customerAudioCtx.createOscillator();
                const gain = customerAudioCtx.createGain();
                osc.type = 'sine';
                osc.frequency.setValueAtTime(freq, customerAudioCtx.currentTime + time);
                gain.gain.setValueAtTime(0.45, customerAudioCtx.currentTime + time);
                gain.gain.exponentialRampToValueAtTime(0.01, customerAudioCtx.currentTime + time + duration);
                osc.connect(gain); gain.connect(customerAudioCtx.destination);
                osc.start(customerAudioCtx.currentTime + time);
                osc.stop(customerAudioCtx.currentTime + time + duration);
            }
            if (status === 'Ready for Pick-up') {
                // Upbeat 3-tone chime for Ready
                playTone(523.25, 0, 0.2);   // C5
                playTone(659.25, 0.22, 0.2); // E5
                playTone(783.99, 0.44, 0.4); // G5
            } else if (status === 'Cancelled') {
                // Low descending tone for Cancelled
                playTone(440, 0, 0.3);   // A4
                playTone(349.23, 0.35, 0.4); // F4
            } else {
                // Gentle single chime for other status changes
                playTone(659.25, 0, 0.25);
                playTone(523.25, 0.3, 0.35);
            }
        } catch(e) { console.error("Customer audio error:", e); }
    }
        ...
        try {
            ...
            data.forEach(serverOrder => {
                ...
                if (localOrder && localOrder.status !== serverOrder.status) {
                    ...
                    playCustomerNotificationSound(serverOrder.status);
                    const statusEmoji = {
                    ...
                    showToast(`${statusEmoji} Order #${serverOrder.code}: ${serverOrder.status}`, serverOrder.status === 'Cancelled' ? 'error' : 'success');
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=DM+Sans:wght@400;500;600;700&family=Poppins:wght@400;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    ...
    <style>
        ...
        * { 
        ...
        .admin-notif-container { position: relative; display: flex; align-items: center; }
        .admin-notif-bell {
            cursor: pointer; position: relative; padding: 9px 12px;
            border-radius: 50px; background: #F5EFE6;
            border: 1px solid #D7CCC8; display: flex; align-items: center; gap: 6px;
            transition: background 0.2s;
        }
        .admin-notif-bell:hover { background: #EFEBE4; }
        .admin-notif-bell i { font-size: 1.1rem; color: #6F4E37; }
        .admin-notif-badge {
            ...
            border: 2px solid white; animation: pulse-badge 1.5s infinite;
        }
        @keyframes pulse-badge { 0%,100% { transform: scale(1); } 50% { transform: scale(1.2); } }
        .admin-notif-dropdown {
            position: absolute; top: calc(100% + 10px); right: 0;
            background: white; border: 1px solid #EFEBE4; border-radius: 14px;
            width: 340px; box-shadow: 0 16px 40px rgba(62,39,35,0.18); z-index: 1000;
            flex-direction: column; overflow: hidden;
        }
        .admin-notif-header {
            padding: 14px 18px; border-bottom: 1px solid #EFEBE4;
            font-family: 'Playfair Display', serif;
            font-weight: 700; font-size: 1rem; color: #3E2723;
            display: flex; justify-content: space-between; align-items: center;
            background: #F5EFE6;
        }
        .admin-notif-clear { font-size: 0.72rem; color: #8D6E63; cursor: pointer; font-weight: 600; font-family: 'DM Sans', sans-serif; }
        .admin-notif-clear:hover { color: #C62828; }
        .admin-notif-list { max-height: 320px; overflow-y: auto; display: flex; flex-direction: column; }
        .admin-notif-item {
            padding: 12px 18px; border-bottom: 1px solid #EFEBE4;
            font-size: 0.85rem; color: #3E2723; font-weight: 600; line-height: 1.45;
            display: flex; align-items: flex-start; gap: 10px;
        }
        .admin-notif-item i { color: #D97706; margin-top: 2px; flex-shrink: 0; }
        .admin-notif-item.new-order i { color: #2E7D32; }
        .admin-notif-empty { padding: 20px; text-align: center; color: #A1887F; font-size: 0.83rem; }

        /* ── STATUS BADGE COLORS ── */
        .status-waiting    { background: #EDE7F6; color: #4527A0; border: 1px solid #D1C4E9; }
        .status-preparing  { background: #FFF3E0; color: #E65100; border: 1px solid #FFE0B2; }
        .status-ready      { background: #E8F5E9; color: #2E7D32; border: 1px solid #C8E6C9; }
        .status-completed  { background: #F5EFE6; color: #8D6E63; border: 1px solid #D7CCC8; }
        .status-cancelled  { background: #FFEBEE; color: #C62828; border: 1px solid #FFCDD2; }

        /* Sidebar header matches customer branding */
        .sidebar-header {
...
<body>
    ...
    <main class="main-content">
        <header class="topbar">
            ...
            <div class="topbar-actions">
                <!-- Admin Notification Bell -->
                <div class="admin-notif-container" id="admin-notif-container">
                    <div class="admin-notif-bell" onclick="toggleAdminNotif()" title="New Order Alerts">
                        <i class="fas fa-bell"></i>
                        <span class="admin-notif-badge" id="admin-notif-badge" style="display:none;">0</span>
                    </div>
                    <div class="admin-notif-dropdown" id="admin-notif-dropdown" style="display:none;">
                        <div class="admin-notif-header">
                            <span>🔔 New Order Alerts</span>
                            <span class="admin-notif-clear" onclick="clearAdminNotifs()">Clear All</span>
                        </div>
                        <div class="admin-notif-list" id="admin-notif-list">
                            <div class="admin-notif-empty">No new orders yet.</div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateAdminNotifUI() {
            ...
            if (adminNotifications.length > 0) {
                list.innerHTML = adminNotifications.map(n => `
                    <div class="admin-notif-item new-order">
                        <i class="fas fa-receipt"></i>
                        <div>New order from <strong>${escapeHTML(n.name)}</strong><br>
                        <span style="color:#6F4E37; font-size:0.78rem;">Order #${escapeHTML(n.code)} · ₱${n.total} · ${escapeHTML(n.time)}</span></div>
                    </div>`).join('');
            } else {
                list.innerHTML = '<div class="admin-notif-empty">No new orders yet.</div>';
            }
        }
        ...
        const ADMIN_ITEM_PHOTOS = {
            ...
            'Cookies & Cream Frappe':  '/static/images/cookies_and_cream_frappe.jpg',
            ...
            'Matcha Frappe':           '/static/images/menu_matcha.jpg',
            'Matcha Caramel':          '/static/images/matcha_caramel.png',
            'Matcha Latte':            '/static/images/matcha_latte.png',
            ...
            'Dirty Matcha':            '/static/images/menu_matcha.jpg',
        ...
        async function fetchLiveOrders() {
            try {
                ...
                data.orders.forEach(order => {
                    ...
                    if (!isInitialLoad && !knownOrderIds.has(order.id) && (order.status === 'Waiting Confirmation' || order.status === 'Preparing Order')) {
                        hasNewOrder = true;
                    }
                    ...
                    let selectHtml = `
                        <select style="padding:8px 12px; border-radius:20px; font-weight:bold; font-family:'Poppins'; font-size:0.8rem; text-transform:uppercase; outline:none; cursor:pointer; ${getStatusClass(order.status)}" onchange="updateOrderStatus(${order.id}, this.value)">
                ...
                if (hasNewOrder) {
                    ...
                    // Push to admin notification bell
                    data.orders.forEach(order => {
                        if (!isInitialLoad && !knownOrderIds.has(order.id) && order.status === 'Waiting Confirmation') {
                            const now = new Date();
                            const timeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                            adminNotifications.unshift({ code: order.code, name: order.name, total: order.total.toFixed(2), time: timeStr });
                            adminUnread += 1;
                        }
                    });
                    if (adminNotifications.length > 15) adminNotifications = adminNotifications.slice(0, 15);
                    updateAdminNotifUI();
                }
