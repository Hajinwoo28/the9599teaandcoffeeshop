...
import random
import io
import requests  # <-- Used to send the Real SMS via API
from flask import (
    Flask, 
...
# --- DIGITAL TRANSFORMATION FEATURE: DATA MIGRATION ---
...
if os.environ.get('RENDER'):
    # Only encrypt cookies strictly if on Render's HTTPS
...
def get_local_ip():
    ...
    Used to generate a direct LAN link for the customer POS if running locally without internet.
    """
    try:
...
def get_ph_time():
    ...
    Ensures all timestamps in the database match local time accurately.
    """
...
class Reservation(db.Model):
    ...
    Tracks the total price, status, and whether it came from the online QR code
    or was entered manually by the staff.
    """
    ...
    # DIGITAL TRANSFORMATION FEATURE 1: ORDER SOURCE
    # Tracks if the order came from the Customer UI ('Online'), 
    # the Admin Manual Entry ('Manual/Notebook'), or a CSV Import ('Legacy Notebook')
...
class RecipeItem(db.Model):
    """
    Maps an ingredient to a specific menu item, defining how much of that 
    ingredient is required to make 1 serving of the drink.
    """
...
# DIGITAL TRANSFORMATION FEATURE 2: PETTY CASH & EXPENSES
...
<body>
    ...
    <!-- ============================================== -->
    <!-- OTP GATEKEEPER SCREEN (Customer verification)  -->
    <!-- ============================================== -->
    <div id="otp-gatekeeper" style="display:flex; height:100vh; width:100vw; justify-content:center; align-items:center; background:#F5EFE6; padding: 20px;">
        <div style="background:white; padding:40px; border-radius:15px; box-shadow:0 10px 25px rgba(0,0,0,0.1); width:100%; max-width:400px; text-align:center; border: 2px solid #6F4E37;">
            ...
            <div id="step-1">
                ...
                <button class="btn-login" onclick="sendOTP()" id="btn-send-otp" style="background:#6F4E37; color:white; border:none; width:100%; padding:15px; border-radius:8px; font-weight:700; cursor:pointer;">
            ...
            <div id="step-2" style="display:none;">
                ...
                <button class="btn-login" onclick="verifyOTP()" id="btn-verify-otp" style="background:#388E3C; color:white; border:none; width:100%; padding:15px; border-radius:8px; font-weight:700; cursor:pointer;">
                    Verify & Enter Shop
                </button>
...
def storefront():
    ...
    try:
        # Decode the token to check validity and extract store hours
        ...
        try:
            ...
            # If current time is strictly greater than the designated closing time, reject
            if current_ph_time > expiry_dt:
                return expired_html, 403
        except Exception as e:
            # Fallback parsing mechanism if exact hour logic fails
...
def send_otp():
    """
    Generates a 6-digit OTP and sends it via SMS API to verify the customer's phone number.
    Limits to 3 requests per minute to prevent spam via firewall.
    """
    data = request.json
    ...
    # Store name temporarily until verified
    session['temp_name'] = name
    db.session.commit()

    SEMAPHORE_API_KEY = os.environ.get('SEMAPHORE_API_KEY', '')
    message = f"9599 Tea & Coffee: Your verification code is {otp_code}. Valid for 5 mins."
    ...
    if SEMAPHORE_API_KEY:
        ...
        # Fallback output to terminal if real API key is absent
...
def verify_otp():
    ...
    If valid, sets a verified session cookie allowing access to the store.
    """
    data = request.json
    phone = data.get('phone')
    code = data.get('code')
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        # We check the raw input against the salted hash stored in memory
...
def reserve_blend():
    ...
    try:
        # Phase 1: Stock Validation & Deduction
        for item in data['items']:
            menu_item = MenuItem.query.filter_by(name=item['foundation']).first()
            if menu_item:
                if menu_item.recipe:
                    # Validate all ingredients before deducting
...
def api_orders():
    ...
    # Filter out archive data and order strictly by descending time limit to 50 items
...
def admin_manual_order():
    ...
    try:
        # Phase 1: Register as 'Manual/Notebook' walk-in record
        res = Reservation(
        ...
        # Phase 2: Add Infusions and Deduct Inventory Automatically
        for item in data['items']:
            inf = Infusion(
            ...
            # Deduct Inventory mathematically instead of writing lines in a book
            menu_item = MenuItem.query.filter_by(name=item['foundation']).first()
        ...
        # Trigger the POS to open the digital receipt
        return jsonify({
            "status": "success", 
            "print_receipt": True
        })
...
def daily_finance():
    ...
    # Define exact bounds of the current calendar day
    start_of_day = now.replace(hour=0, minute=0, second=0, microsecond=0)
    end_of_day = now.replace(hour=23, minute=59, second=59, microsecond=999999)
    
    # Calculate System Total (Online + Manual POS, specifically excluding bulk legacy imports)
    ...
    # Aggregate all "Money Out" via Petty Cash Expenses for today
...
def add_expense():
    ...
    Example: Buying emergency ice or milk from the market using the register.
    """
    if not session.get('is_admin'): 
        return jsonify({"error": "Unauthorized"}), 403
...
def import_legacy():
    ...
    if not pd: 
        return jsonify({"error": "The 'pandas' library is required to process imports. Please install it on the server."}), 500
    ...
    try:
        # Utilize pandas for safe, reliable dataframe iteration
        df = pd.read_csv(file)
        imported_count = 0
        
        for index, row in df.iterrows():
            # Graceful time fallback
            try: 
            ...
            # Insert as Legacy Archive Source
            res = Reservation(
...
def import_backup():
    ...
    try:
        ...
        # Reset sequences for Postgres integrity to ensure primary keys don't collide
...
with app.app_context():
    try:
        ...
        # Only populate if database is brand new (Prevents duplicate entries on restart)
...
if __name__ == '__main__':
    # Detect if we are running in the cloud (Render/Heroku/Railway)
    if os.environ.get('RENDER') or os.environ.get('DYNO'):
        app.run(host='0.0.0.0', port=5000)
    else:
        # Desktop / Offline LAN Mode
        try:
            import webview
            ...
            def start_server():
                ...
                This ensures the web server dies immediately when the Desktop App window is closed.
                """
            ...
            # This blocking call starts the desktop GUI
            webview.start()
            
        except ImportError:
            # Fallback to pure browser mode if webview isn't installed
