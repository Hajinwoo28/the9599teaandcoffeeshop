                    ...
                    <div class="settings-card" style="padding:20px; margin-bottom:0; flex-shrink:0;">
                        ...
                        <div style="background:#F5EFE6; border:1px solid #EFEBE4; border-radius:12px; padding:16px; margin-bottom:0;">
                            ...
                            <div style="position:relative;">
                                <input type="text" id="posLink" class="input-pin" style="background:#fff; padding-right:44px; font-size:0.78rem; color:#6F4E37;" readonly placeholder="Your permanent link will appear here…">
                                <button onclick="copyPosLink()" title="Copy link" style="position:absolute; right:6px; top:50%; transform:translateY(-50%); background:#6F4E37; border:none; border-radius:6px; color:#fff; width:32px; height:32px; cursor:pointer; display:flex; align-items:center; justify-content:center;">
...
with app.app_context():
    try:
        ...
        try:
            db.session.execute(db.text('ALTER TABLE menu_items ADD COLUMN is_out_of_stock BOOLEAN NOT NULL DEFAULT 0'))
            db.session.commit()
        except Exception:
            db.session.rollback()
