...
if os.environ.get('RENDER') or os.environ.get('DYNO'):
    ...
    # Lax + Secure=False is the only combination that works reliably in
    # both pywebview's embedded WebKit and a normal desktop browser.
    # SameSite=None requires Secure=True (HTTPS) which we don't have locally.
...
# Explicitly use in-memory storage for rate limiting.
# This silences the production warning while keeping the same behaviour.
# To use Redis in production, replace the storage_uri with "redis://localhost:6379".
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── SEARCH BAR (matches admin qo-search-bar style) ── */
        .menu-search-bar {
            width: 100%;
            ...
            box-shadow: 0 2px 8px rgba(30,15,8,0.04);
        }
        ...
        .menu-search-clear.visible { display: flex; }
        .menu-search-clear:hover { background: var(--latte); color: var(--coffee); }
...
<div class="main-container">
    ...
    <div class="menu-area">
        ...
        <div class="menu-search-wrapper">
            ...
            <input type="text" class="menu-search-bar" id="menu-search-input"
                   placeholder="Search drinks, snacks…"
                   oninput="onMenuSearch(this.value)"
                   autocomplete="off">
            <button class="menu-search-clear" id="menu-search-clear" onclick="clearMenuSearch()" title="Clear">
...
<div id="perm-modal" class="modal">
    <div class="modal-content" style="text-align:center; max-width:440px; width:92vw;">
        ...
        <p style="color:var(--toffee); font-size:0.85rem; line-height:1.55; margin-bottom:14px;">
            You can only order up to <strong>4 milktea drinks</strong> without permission.<br>
            Fill in the form below and send a request to the admin to be allowed to order 5 or more.
        </p>
        <div class="perm-code-box" id="perm-request-code">—</div>

        <!-- Address Field -->
        <div style="text-align:left; margin-top:14px; margin-bottom:10px;">
            <label style="font-size:0.75rem; font-weight:800; color:var(--toffee); text-transform:uppercase; display:block; margin-bottom:6px;">
                📍 Your Address / Where are you from?
            </label>
            <input type="text" id="perm-address-input"
                placeholder="e.g. Brgy. San Antonio, San Pablo City"
                style="width:100%; padding:12px 14px; border:1.5px solid var(--foam); border-radius:10px;
                       font-family:'DM Sans',sans-serif; font-size:0.88rem; color:var(--espresso);
                       background:var(--milk); outline:none; box-sizing:border-box;"
                onfocus="this.style.borderColor='var(--caramel)'"
                onblur="this.style.borderColor='var(--foam)'">
        </div>
        ...
        <div style="text-align:left; margin-bottom:14px;">
            ...
            <textarea id="perm-message-input" rows="3"
                placeholder="e.g. Hi! I'd like to order 7 milkteas for our office. Please allow us."
                style="width:100%; padding:12px 14px; border:1.5px solid var(--foam); border-radius:10px;
                       font-family:'DM Sans',sans-serif; font-size:0.88rem; color:var(--espresso);
                       background:var(--milk); outline:none; resize:none; box-sizing:border-box;"
                onfocus="this.style.borderColor='var(--caramel)'"
                onblur="this.style.borderColor='var(--foam)'"></textarea>
        </div>

        <div id="perm-send-status" style="font-size:0.8rem; color:var(--sage); font-weight:700; min-height:18px; margin-bottom:6px;"></div>

        <p style="font-size:0.75rem; color:var(--toffee); margin-bottom:10px;">
            Waiting for the admin to approve… this page will update automatically.
        </p>

        <div style="display:flex; gap:10px; margin-top:8px;">
            <button class="btn-cancel" onclick="closePermModal()">Cancel</button>
            <button class="btn-add" id="perm-send-btn" onclick="sendPermissionRequest()" style="flex:2;">
                <i class="fas fa-paper-plane"></i> Send Request
            </button>
            <button class="btn-add" id="perm-place-btn" style="display:none; flex:2;" onclick="submitOrderWithOverride()">
...
<script>
    ...
    function renderMenu(categoryFilter) {
        ...
        grid.innerHTML = '';
        let filteredItems = categoryFilter === 'All' ? menuItems : menuItems.filter(item => item.category === categoryFilter);
        // apply search query
        if (menuSearchQuery) {
        ...
        if (filteredItems.length === 0) {
            grid.innerHTML = menuSearchQuery
                ? `<div class="empty-category"><i class="fas fa-search" style="font-size:1.3rem; opacity:0.3;"></i><p style="margin-top:10px;">No results for "<strong>${escapeHTML(menuSearchQuery)}</strong>"</p></div>`
                : '<div class="empty-category">No items in this category.</div>';
            return;
        }
    ...
    function onMenuSearch(val) {
        ...
        if (clearBtn) clearBtn.classList.toggle('visible', menuSearchQuery.length > 0);
    ...
    function closePermModal() {
        ...
        const addrEl = document.getElementById('perm-address-input');
        const msgEl  = document.getElementById('perm-message-input');
    ...
    async function sendPermissionRequest() {
        ...
        const reqCode = document.getElementById('perm-request-code').innerText;
        const statusEl = document.getElementById('perm-send-status');
        const sendBtn  = document.getElementById('perm-send-btn');

        if (!address) {
            statusEl.style.color = 'var(--red-alert)';
            statusEl.innerText = '⚠️ Please enter your address first.';
            return;
        }
        if (!message) {
            statusEl.style.color = 'var(--red-alert)';
            statusEl.innerText = '⚠️ Please write a message to the admin.';
            return;
        }

        sendBtn.disabled = true;
        sendBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Sending…';
        statusEl.style.color = 'var(--toffee)';
        statusEl.innerText = 'Sending your request…';

        try {
            const res = await fetch('/api/permission_request', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, address, message, code: reqCode })
            });
            if (res.ok) {
                statusEl.style.color = 'var(--sage)';
                statusEl.innerText = '✅ Request sent! Waiting for admin approval…';
                sendBtn.innerHTML = '<i class="fas fa-check"></i> Sent';
                playPermissionRequestSound();
                showToast('📨 Permission request sent to admin!', 'success');
    ...
    async function submitOrder() {
        ...
        // Check milktea order limit (5 milktea max without permission)
        const milkteaCount = countMilkteaInCart();
        if (milkteaCount >= MILKTEA_LIMIT && !permissionGranted) {
            // Generate a simple request code
            const reqCode = name.toUpperCase().replace(/\s+/g,'').slice(0,4)
                          + '-MT' + String(milkteaCount).padStart(2,'0');
            ...
            // Reset send button and status for fresh request
            const sendBtn = document.getElementById('perm-send-btn');
            ...
            // Play urgent sound
            playPermissionRequestSound();
            showToast(`🛑 ${MILKTEA_LIMIT} or more milkteas — ask staff for permission.`, 'error');
...
<head>
    ...
    <style>
        ...
        * { 
        ...
        /* Manual POS Search Bar — mirrors customer site menu-search-bar */
        .qo-search-wrapper {
            position: relative;
            margin-bottom: 15px;
        }
        ...
        .qo-search-bar:focus {
            border-color: #6F4E37;
            background: #fff;
            box-shadow: 0 2px 12px rgba(111,78,55,0.12);
        }
        ...
        /* Manual POS Cards — matches customer site card design */
        .qo-card { 
            background: #FDFBF7;
        ...
        .qo-card:hover {
            ...
            box-shadow: 0 8px 24px rgba(30,15,8,0.12);
        }
        .qo-card:active { transform: scale(0.97); }
        .qo-card.sold-out { opacity: 0.45; cursor: not-allowed; pointer-events: none; }

        /* Image container — mirrors .card-img-container */
        .qo-card-img-wrap {
            height: 128px;
            position: relative;
            display: flex; justify-content: center; align-items: center;
        ...
        .qo-card-img-wrap.cat-trending   { background: linear-gradient(145deg, #4A1C10, #C8922A); }
        .qo-card-img-wrap.cat-signature  { background: linear-gradient(145deg, #2D1B45, #7B5EA7); }
        .qo-card-img-wrap.cat-classic    { background: linear-gradient(145deg, #3B2010, #9B6A3A); }
        .qo-card-img-wrap.cat-matcha     { background: linear-gradient(145deg, #1A3020, #5C7C5C); }
        .qo-card-img-wrap.cat-fruit      { background: linear-gradient(145deg, #4A1028, #C0395A); }
        .qo-card-img-wrap.cat-taro       { background: linear-gradient(145deg, #2E1A45, #8B6AB5); }
        .qo-card-img-wrap.cat-snacks     { background: linear-gradient(145deg, #3B2B08, #C87F20); }
        .qo-card-img-wrap.cat-default    { background: linear-gradient(145deg, #2E1808, #8C4A30); }
        .qo-card-img-wrap.cat-milk       { background: linear-gradient(145deg, #1A2E45, #4A8BA8); }
        .qo-card-img-wrap.cat-soda       { background: linear-gradient(145deg, #0D3B26, #2E9C6A); }
        .qo-card-img-wrap.cat-frappe     { background: linear-gradient(145deg, #1A1A2E, #5C4A8A); }
        ...
        .qo-card:hover .qo-card-photo { transform: scale(1.06); }
        .qo-card-overlay {
            position: absolute; bottom: 0; left: 0; right: 0; height: 55%;
            background: linear-gradient(to top, rgba(20,8,4,0.72) 0%, transparent 100%);
            z-index: 1;
        ...
        .qo-card-price-badge {
            ...
            background: rgba(0,0,0,0.55); backdrop-filter: blur(4px);
            color: #C8922A; padding: 4px 9px; border-radius: 20px;
            font-weight: 700; font-size: 0.78rem; letter-spacing: 0.3px;
        }
        .qo-card-sold-out-badge {
            position: absolute; top: 0; left: 0; width: 100%; height: 100%;
            background: rgba(255,255,255,0.55); backdrop-filter: blur(2px);
            display: flex; justify-content: center; align-items: center;
            font-weight: 900; color: #C0392B;
            font-size: 0.9rem; z-index: 10; letter-spacing: 2px; text-transform: uppercase;
        }
        .qo-card-low-stock {
        ...
        .qo-card-name {
            font-weight: 700; color: #1E0F08;
            font-size: 0.88rem; line-height: 1.25;
        }
        .qo-card-cat {
            margin-top: 5px; font-size: 0.68rem; font-weight: 600;
            color: #A67B5B; text-transform: uppercase; letter-spacing: 0.8px;
        }
...
<body>
    ...
    <aside class="sidebar">
        <div>
            ...
            <nav class="nav-links">
                ...
                <div class="nav-item" onclick="switchTab('perm-requests', 'Permission Requests', this)" id="nav-perm-requests">
                    <i class="fas fa-user-lock"></i> Permissions
                    <span id="perm-req-badge" style="display:none; background:#E65100; color:white; font-size:0.65rem; font-weight:800; border-radius:50%; width:18px; height:18px; line-height:18px; text-align:center; margin-left:auto; flex-shrink:0;"></span>
                </div>
    ...
    <main class="main-content">
        ...
        <div class="content-body">
            ...
            <!-- PERMISSION REQUESTS                            -->
            <!-- ============================================== -->
            <div id="tab-perm-requests" class="tab-pane">
                <div class="settings-card" style="height:100%; display:flex; flex-direction:column; min-height:0;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-shrink:0; margin-bottom:15px;">
                        <div>
                            <div class="card-title" style="margin-bottom:4px;"><i class="fas fa-user-lock"></i> Customer Permission Requests</div>
                            <p class="desc-text" style="margin-bottom:0;">Customers who have exceeded the 5-milktea limit and are requesting approval to order more.</p>
                        </div>
                        <button class="btn-dark" onclick="fetchPermissionRequests()"><i class="fas fa-sync"></i> Refresh</button>
                    </div>
                    <div class="table-responsive" style="margin-top:0; flex:1; overflow-y:auto;">
                        <table class="kds-table">
                            <thead>
                                <tr>
                                    <th>Time</th>
                                    ...
                                    <th>✉️ Message to Admin</th>
                                    <th style="text-align:center;">Action</th>
                                </tr>
                            </thead>
                            <tbody id="perm-requests-table-body">
                                <tr><td colspan="6" style="text-align:center; padding:40px; color:#A1887F; font-weight:600;">No pending permission requests.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div id="tab-quick-order" class="tab-pane">
                <div class="quick-order-layout" style="flex:1; min-height:0; overflow:hidden;">
                    <div style="display:flex; flex-direction:column; overflow:hidden; min-height:0;">
                        <!-- Search Bar with Icon -->
                        <div class="qo-search-wrapper">
                            <i class="fas fa-search qo-search-icon"></i>
            ...
            <div id="tab-finance" class="tab-pane">
                <div class="finance-grid" style="overflow-y:auto; flex:1; min-height:0; align-content:start;">
            ...
            <div id="tab-settings" class="tab-pane">
                <div class="settings-grid-layout" style="overflow-y:auto; flex:1; min-height:0;">
    ...
    <script>
        ...
        // Returns the Response on success, null on any failure.
        // Never redirects — Flask guards /admin server-side.
        async function apiFetch(url, options = {}) {
            try {
                const res = await fetch(url, Object.assign({ credentials: 'same-origin' }, options));
                if (!res.ok && res.status === 403) {
                    console.warn('apiFetch 403 — session expired for:', url);
        ...
        function updateTime() {
            try {
                ...
                if (clock) clock.innerText = hours + ':' + minutes + ' ' + ampm;
        ...
        // Session is validated server-side by Flask before rendering this page.
        // No client-side ping needed — if the user sees this page, they are authenticated.
        ...
        function switchTab(tabId, title, btnElement) {
            try {
                ...
                if (!targetPane) { console.error('Tab pane not found: tab-' + tabId); return; }
                ...
                const loaders = {
                    'kds':           fetchLiveOrders,
                    'quick-order':   fetchQuickOrderMenu,
                    'inventory':     fetchAdminInventory,
                    'finance':       fetchDailyFinances,
                    'audit':         fetchAuditLogs,
                    'settings':      fetchAdminMenu,
                    'perm-requests': fetchPermissionRequests
                };
        ...
        let _kdsRetryCount = 0;

        async function fetchLiveOrders() {
            const tbody = document.getElementById('kds-table-body');
            if (!tbody) return;

            // Show a loading row only on the very first call
            if (isInitialLoad && tbody.innerHTML.trim() === '') {
                tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:40px; color:#A1887F; font-weight:600;"><i class="fas fa-spinner fa-spin" style="margin-right:8px;"></i>Loading orders…</td></tr>';
            }

            try {
                const response = await apiFetch('/api/orders?_t=' + new Date().getTime());
                if (!response) return;
                const data = await response.json();
                _kdsRetryCount = 0; // reset on success
                tbody.innerHTML = '';
                    ...
                    tbody.innerHTML += `
                        <tr style="${order.over_limit ? 'background:#FFF8F0; border-left:3px solid #E65100;' : ''}">
                            ...
                            <td style="font-weight:800; color:#3E2723;">
                                ...
                                ${order.over_limit ? `<br><button onclick="grantPermission('${escapeHTML(order.name)}')" style="margin-top:6px; background:#E65100; color:white; border:none; padding:5px 10px; border-radius:20px; font-size:0.72rem; font-weight:700; cursor:pointer;">🔓 Grant +5 Permission</button>` : ''}
                            </td>
                ...
                _kdsRetryCount++;
                if (tbody && isInitialLoad) {
                    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:40px; color:#A1887F; font-weight:600;"><i class="fas fa-wifi-slash" style="font-size:1.4rem; display:block; margin-bottom:10px; opacity:0.5;"></i>Connecting to server\u2026 (attempt ' + _kdsRetryCount + ')<br><span style="font-size:0.8rem; font-weight:500; margin-top:6px; display:block;">This will refresh automatically.</span></td></tr>';
                }
            }
        }
        ...
        // Start polling — small initial delay so pywebview's session cookie
        // is fully established before the first API request fires.
        setTimeout(function() {
            fetchLiveOrders();
            setInterval(fetchLiveOrders, 3000);
        }, 600);
        // Also poll permission-request badge (staggered slightly)
        setTimeout(function() {
        ...
        async function grantPermission(customerName) {
            try {
                const res = await apiFetch('/api/admin/grant_permission', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (res.ok) {
                    ...
                    fetchPermissionRequests(); // refresh requests table
                } else {
        ...
        // ----------------------------------------
        // PERMISSION REQUESTS TABLE (Admin Panel)
        // ----------------------------------------
        async function fetchPermissionRequests() {
            try {
                const res = await apiFetch('/api/permission_requests?_t=' + Date.now());
                if (!res) return;
                const requests = await res.json();

                const badge = document.getElementById('perm-req-badge');
                if (requests.length > 0) {
                    badge.style.display = 'inline-block';
                    badge.innerText = requests.length;
                    // Play alert if new requests appeared
                } else {
                    badge.style.display = 'none';
                }

                const tbody = document.getElementById('perm-requests-table-body');
                if (!tbody) return;

                if (requests.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:40px; color:#A1887F; font-weight:600;">No pending permission requests.</td></tr>';
                    return;
                }
                ...
                requests.forEach(req => {
                    tbody.innerHTML += `
                        <tr style="background:#FFF8F0; border-left:3px solid #E65100;">
                            <td style="color:#8D6E63; font-size:0.8rem; white-space:nowrap;">${escapeHTML(req.ts || '—')}</td>
                            <td style="font-weight:800; color:#3E2723;">${escapeHTML(req.name)}</td>
                            <td><span class="kds-badge" style="background:#FFF3E0; color:#E65100; border:1px solid #FFE0B2; font-family:monospace; font-size:0.85rem;">${escapeHTML(req.code || '—')}</span></td>
                            <td style="color:#5D4037; font-size:0.88rem;">
                                <i class="fas fa-map-marker-alt" style="color:#E65100; margin-right:5px;"></i>${escapeHTML(req.address)}
                            </td>
                            <td style="color:#5D4037; font-size:0.88rem; max-width:260px; white-space:pre-wrap; word-break:break-word;">${escapeHTML(req.message)}</td>
                            <td style="text-align:center;">
                                <button onclick="grantPermission('${escapeHTML(req.name).replace(/'/g,"\\'")}')"
                                    style="background:#388E3C; color:white; border:none; padding:8px 16px; border-radius:20px;
        ...
        async function fetchQuickOrderMenu() {
            try {
                const res = await apiFetch('/api/menu?_t=' + new Date().getTime());
                if (!res) return;
        ...
        function renderQuickOrderMenu(items) {
            ...
            if (items.length === 0) {
                grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; color:#A1887F; padding:40px 0;"><i class="fas fa-search" style="font-size:1.3rem; opacity:0.3; display:block; margin-bottom:8px;"></i>No items found.</div>';
                return;
            }

            // Helper: get category CSS class (mirrors getCatInfo on customer site)
            function getQOCatInfo(cat) {
                const map = {
                    'Trending Now':    { cls: 'cat-trending',  emoji: '🔥' },
                    'Signature Series':{ cls: 'cat-signature', emoji: '✨' },
                    'Classic Milk Tea':{ cls: 'cat-classic',   emoji: '🧋' },
                    'Matcha Menu':     { cls: 'cat-matcha',    emoji: '🍵' },
                    'Matcha & Taro':   { cls: 'cat-matcha',    emoji: '🍵' },
                    'Fruit Infusions': { cls: 'cat-fruit',     emoji: '🍓' },
                    'Milk Series':     { cls: 'cat-milk',      emoji: '🥛' },
                    'Fruit Soda':      { cls: 'cat-soda',      emoji: '🧃' },
                    'Frappes':         { cls: 'cat-frappe',    emoji: '🧊' },
                    'Snacks':          { cls: 'cat-snacks',    emoji: '🍟' },
                };
                return map[cat] || { cls: 'cat-default', emoji: '🧋' };
            }
            
            items.forEach(i => {
                const fixedCats = ['Snacks','Frappes','Fruit Soda','Milk Series'];
                const priceDisplay = fixedCats.includes(i.category) ? `₱${i.price.toFixed(0)}` : '₱49 / ₱59';
                const onClick = `addToQuickCart('${i.name.replace(/\\/g,'\\\\').replace(/'/g,"\\'")}', '${i.category.replace(/'/g,"\\'")}', ${i.price})`;
                const info = getQOCatInfo(i.category);
                const isSoldOut = i.stock <= 0;

                // Photo
                const imgSrc = ADMIN_ITEM_PHOTOS[i.name] || null;
                const photoHTML = imgSrc
                    ? `<img class="qo-card-photo" src="${imgSrc}" alt="${escapeHTML(i.name)}" onerror="this.style.display='none'">`
                    : '';

                // Badges
                let badgeHTML = '';
                if (isSoldOut) {
                    badgeHTML = `<div class="qo-card-sold-out-badge">SOLD OUT</div>`;
                } else if (i.stock > 0 && i.stock <= 5) {
                    badgeHTML = `<div class="qo-card-low-stock">⚡ ${i.stock} left</div>`;
                }

                grid.innerHTML += `
                    <div class="qo-card${isSoldOut ? ' sold-out' : ''}" onclick="${isSoldOut ? '' : onClick}">
                        <div class="qo-card-img-wrap ${info.cls}">
                            ${photoHTML}
                            <div class="qo-card-overlay"></div>
                            ${badgeHTML}
                            <div class="qo-card-price-badge">${priceDisplay}</div>
                        </div>
                        <div class="qo-card-body">
                            <div class="qo-card-name">${escapeHTML(i.name)}</div>
                            <div class="qo-card-cat">${escapeHTML(i.category)}</div>
                        </div>
                    </div>
                `;
            });
        ...
        async function submitQuickOrder() {
            ...
            try {
                const res = await apiFetch('/api/admin/manual_order', {
                    method: 'POST', 
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
        ...
        async function fetchAdminInventory() {
            ...
            if (tbodyInv) tbodyInv.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:30px;color:#A1887F;"><i class="fas fa-spinner fa-spin"></i> Loading inventory…</td></tr>';
            try {
                const res = await apiFetch('/api/inventory?_t=' + new Date().getTime());
                if (!res) return;
                ...
                if (!ings || ings.length === 0) {
                    tbodyInv.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:30px;color:#A1887F;font-weight:600;">No ingredients found.</td></tr>';
                    return;
                }
                ...
                console.error(error);
                if (tbodyInv) tbodyInv.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:30px;color:#C62828;font-weight:600;"><i class="fas fa-exclamation-circle"></i> Failed to load inventory. Click Refresh.</td></tr>';
            }
        }
        ...
        async function fetchMenuTrends() {
            try {
                const res = await apiFetch('/api/finance/menu_trends');
                if (!res) return;
                const data = await res.json();
        ...
        async function uploadLegacyCSV() {
            ...
            try {
                const res = await apiFetch('/api/import_legacy', { 
                    method: 'POST', 
                    body: formData 
                });
                if (!res) return;
        ...
        async function fetchAuditLogs() {
            ...
            if (tbody) tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:30px;color:#A1887F;"><i class="fas fa-spinner fa-spin"></i> Loading audit logs…</td></tr>';
            try {
                const res = await apiFetch('/api/audit_logs?_t=' + new Date().getTime());
                if (!res) return;
                ...
                if (logs.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:30px;color:#A1887F;font-weight:600;">No audit logs available.</td></tr>';
                    return;
                }
                ...
                if (tbody) tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:30px;color:#C62828;font-weight:600;"><i class="fas fa-exclamation-circle"></i> Failed to load logs. Click Refresh.</td></tr>';
            }
        }
        ...
        async function fetchAdminMenu() {
            ...
            if (tbodyMenu) tbodyMenu.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:30px;color:#A1887F;"><i class="fas fa-spinner fa-spin"></i> Loading menu…</td></tr>';
            try {
                const res = await apiFetch('/api/menu?_t=' + new Date().getTime());
                if (!res) return;
                ...
                if (items.length === 0) { 
                    tbodyMenu.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:30px;color:#A1887F;font-weight:600;">No items in menu.</td></tr>'; 
                    return; 
                }
                ...
                if (tbodyMenu) tbodyMenu.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:30px;color:#C62828;font-weight:600;"><i class="fas fa-exclamation-circle"></i> Failed to load menu. Click refresh.</td></tr>';
            }
        }
        ...
        async function saveConfigurations() {
            ...
            try {
                const res = await apiFetch('/api/generate_link', {
                    method: 'POST', 
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
...
_permission_requests = []    # list of { name, address, message, code, ts } from customers

@app.route('/api/ping')
def api_ping():
    """
    Lightweight session health-check. The admin frontend calls this on startup
    to verify the session cookie arrived correctly (critical for pywebview).
    """
...
def desktop_login():
    """
    Previously used for pywebview auto-login. Now disabled — the PIN screen
    is always required even on desktop, so this simply redirects to /login.
    """
...
def api_login():
    """
    JSON-based login endpoint used by the AJAX login form.
    Returns JSON so pywebview can handle the response with fetch + credentials:include,
    ensuring the session cookie is properly stored in the webview.
    """
    ...
    if check_password_hash(ADMIN_PIN_HASH, pin):
        ...
        log_audit("Admin Login", "Successful login to dashboard (AJAX)")
        return jsonify({"ok": True})
    else:
        log_audit("Admin Login Failed", "Invalid PIN attempt (AJAX)")
        return jsonify({"ok": False, "error": "Invalid PIN. Access Denied."}), 401

@app.route('/login', methods=['GET', 'POST'])
@limiter.limit("10 per minute")
def admin_login():
    """
    Administrative gateway. Renders the login page (GET).
    POST validates PIN and sets the session, then redirects to /admin.
    """
    ...
    if request.method == 'POST':
        ...
        if pin and check_password_hash(ADMIN_PIN_HASH, pin):
            session.permanent = True
            session['is_admin'] = True
    ...
    resp = make_response(render_template_string(LOGIN_HTML, error=error))
    resp.headers['Cache-Control'] = 'no-store'
    return resp
...
# ── CUSTOMER SUBMITS A PERMISSION REQUEST ──
@app.route('/api/permission_request', methods=['POST'])
def submit_permission_request():
    """
    Customer-facing endpoint. Stores the customer's address and message so the admin
    can see it on the Live Orders panel and grant permission with one click.
    """
    global _permission_requests
    data = request.json or {}
    ...
    # Keep only the latest request per customer (overwrite duplicates)
    _permission_requests = [r for r in _permission_requests if r['name'].upper() != name.upper()]
    _permission_requests.append({
        'name':    name,
        'address': address,
        'message': message,
        'code':    code,
        'ts':      get_ph_time().strftime('%I:%M %p')
    })
    # Cap at 50 pending requests
    if len(_permission_requests) > 50:
        _permission_requests = _permission_requests[-50:]

    log_audit("Permission Request", f"{name} ({address}) requested over-limit order")
    return jsonify({"status": "sent"})

# ── ADMIN FETCHES ALL PENDING PERMISSION REQUESTS ──
@app.route('/api/permission_requests')
def get_permission_requests():
    """
    Admin-only endpoint. Returns all pending customer permission requests
    so the admin can view address, message, and grant permission from the panel.
    """
    if not session.get('is_admin'):
        return jsonify({"error": "Unauthorized"}), 403
    return jsonify(_permission_requests)
...
def api_orders():
    ...
    for res in all_reservations:
        ...
        orders_data.append({
            ...
            'over_limit': len(res.infusions) > 5,
            'items': items
        })
...
if __name__ == '__main__':
    if os.environ.get('RENDER') or os.environ.get('DYNO'):
        # Get Render's dynamic port, default to 5000 if not found
        ...
        try:
            ...
            # Wait until Flask is actually accepting connections before opening webview
            import socket as _socket
            for _attempt in range(30):
                try:
                    _s = _socket.create_connection(('localhost', 5000), timeout=1)
                    _s.close()
                    break
                except OSError:
                    _time.sleep(0.3)
            ...
            # Always show the PIN login screen — no auto-bypass on desktop.
            admin_url = "http://localhost:5000/login"
